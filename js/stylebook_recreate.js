 ...
 * ComfyUI-Manager (js/node_fixer.js), and on ComfyUI frontend 1.47 its
 * implementation throws before it finishes:
 *
 ...
 * `findInputSlot` on it.
 *
 * The callback creates the replacement node first and removes the
 * original last, so the exception leaves both on the canvas: the original
 * still wired up, the replacement unconnected and sitting on top of it.
 * That is the whole of the reported bug, and it is not specific to this
 * pack. It happens to any node with a connected input.
 *
 ...
 * for Stylebook node instances only. Nothing global is patched and no
...
function copyWidgetValues(from, to) {
  ...
  for (const widget of from.widgets || []) {
    if (!widget || !widget.name || widget.serialize === false) continue;
