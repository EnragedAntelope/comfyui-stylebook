...
**Deep references:**
- `ARCHITECTURE.md` (chain protocol, layout, design rationale â€” read before engine changes)
...
- **Gallery-first UX.** Each style ships a rendered preview image. Open gallery â†’ look â†’ click. 850+ artists each have a written descriptor so the look lands even when the model doesn't know the name.
...
- **Generated JS data.** `js/stylebook_data.js` is generated â€” never edit by hand. `js/stylebook_gallery.js` is hand-written and the generator never touches it.
...
| `js/stylebook_data.json` | Generated corpus, fetched on first picker open â€” a `.json` so ComfyUI's `**/*.js` glob does not parse it at app start |
...
- Zero dependencies. Python â‰¥3.10. Drops into ComfyUI's `custom_nodes/` â€” no pip install.
- The chain socket type (`STYLEBOOK_CHAIN`) is distinct from STRING by design â€” prevents silent miswiring when connecting prompt to style_chain.
...
- `user_styles.json` is optional â€” `data/user_data.py` validates and merges it.
...
**No-overlap rule:** Explanatory prose lives in one file. AGENTS.md = agent-facing summary; `ARCHITECTURE.md` = deep reference; `docs/custom-styles.md` = field reference. Identical build/test commands may be restated verbatim. Explanatory prose must not be duplicated â€” link instead.
